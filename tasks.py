...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-indian-currency runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-indian-currency.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_indian_currency")
...
def lint(c):
    ...
    """
    c.run("flake8 django-indian-currency tests")
...
def release(c, bumpsize=''):
    ...
    import django_indian_currency
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_indian_currency.__version__))
    c.run("git push --tags")
